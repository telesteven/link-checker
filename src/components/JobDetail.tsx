      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm p-6">
        ...
        {job.status === "done" && (job.snapshots.pdf || job.snapshots.html) && (
          <div className="mt-4 flex gap-3 text-sm">
            {job.snapshots.pdf && (
              <a className="text-indigo-600 hover:underline" href={api.snapshotUrl(job.id, "pdf")} target="_blank" rel="noreferrer">
                Download PDF
              </a>
            )}
            {job.snapshots.html && (
              <a className="text-indigo-600 hover:underline" href={api.snapshotUrl(job.id, "html")} target="_blank" rel="noreferrer">
                Download HTML
              </a>
            )}
          </div>
      ...
      {job.status === "done" && (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
          <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap items-center gap-3">
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="text-sm rounded border-slate-300">
            ...
            <input
              ...
              className="text-sm rounded border-slate-300 w-44"
            />
            <a
                ...
                {links.map((link, i) => (
                  <tr key={i} className="hover:bg-slate-50">
                    <td className="px-4 py-2 max-w-md truncate" title={link.href}>
                      <a href={link.href} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline">
                        {link.href}
                      </a>
                    </td>
                    <td className="px-4 py-2 text-slate-500">{link.rootDomain}</td>
                    <td className="px-4 py-2">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${link.isInternal ? "bg-slate-100 text-slate-600" : "bg-sky-100 text-sky-700"}`}>
...
function StatusBadge({ status }: { status: number | null }) {
  if (status === null) return <span className="text-xs text-slate-400">unreachable</span>;
  const ok = status >= 200 && status < 400;
  return (
    <span className={`text-xs px-2 py-0.5 rounded-full ${ok ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
      {status}
    </span>
  );
}
...
function SnapshotPreview({
  ...
  if (!available) {
    return (
      <div className="rounded-lg border border-slate-200 bg-slate-50 aspect-video flex items-center justify-center text-slate-400 text-sm">
        {label} unavailable
      </div>
    );
  }
